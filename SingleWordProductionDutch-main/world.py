...
def world(x, fs=44100, frame_period=10):
    '''
...
def code_world_params(f0, sp, ap, fs=44100, num_mfcc=25):
    '''
...
def decode_world_params(lf0, vuv, mfcc, bap, fs=44100):
    '''
...
def synthesise(f0, sp, ap, fs=44100, frame_period=10):
    '''
...
if __name__ == '__main__':
    ...
    if ext == 'wav':
        for filepath in iglob(os.path.join(src_dir, "*.wav")):
            ...
            np.save(os.path.join(trg_dir, filename + "_lf0.npy"), lf0)
            np.save(os.path.join(trg_dir, filename + "_vuv.npy"), vuv)
            np.save(os.path.join(trg_dir, filename + ".npy"), mfcc)
            np.save(os.path.join(trg_dir, filename + "_bap.npy"), bap)
