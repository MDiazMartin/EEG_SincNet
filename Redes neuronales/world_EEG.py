...
path_param_sint = '..\\SingleWordProductionDutch-main\\HUVN\\'
# Sujetos para seleccionar: M11, F09
sub = 'F09'
...
if __name__ == '__main__':
    ...
    if ext == 'wav':
        ...
        for filepath in iglob(os.path.join(src_dir, "*.npy")):
            ...
            # mfcc = x
            # if os.path.exists(os.path.join(path_param_sint, sub, "ses01", "mfcc", os.path.basename(filepath))):
            #     bap = np.load(os.path.join(path_param_sint, sub, "ses01", "mfcc", os.path.basename(filepath).split('.')[0] + "_bap.npy"))
            #     lf0 = np.load(os.path.join(path_param_sint, sub, "ses01", "mfcc", os.path.basename(filepath).split('.')[0] + "_lf0.npy"))
            #     vuv = np.load(os.path.join(path_param_sint, sub, "ses01", "mfcc", os.path.basename(filepath).split('.')[0] + "_vuv.npy"))
            # elif os.path.exists(os.path.join(path_param_sint, sub, "ses02", "mfcc", os.path.basename(filepath))):
            #     bap = np.load(os.path.join(path_param_sint, sub, "ses02", "mfcc", os.path.basename(filepath).split('.')[0] + "_bap.npy"))
            #     lf0 = np.load(os.path.join(path_param_sint, sub, "ses02", "mfcc", os.path.basename(filepath).split('.')[0] + "_lf0.npy"))
            #     vuv = np.load(os.path.join(path_param_sint, sub, "ses02", "mfcc", os.path.basename(filepath).split('.')[0] + "_vuv.npy"))
            # else:
            #     nframes = x.shape[0]
            #     bap = np.zeros(shape=(nframes, 1), dtype=float)
            #     lf0 = np.zeros_like(bap)
